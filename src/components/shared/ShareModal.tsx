...
const ShareModal = ({ type, userId, contentId, open, setOpen }: ShareModalProps) => {
    ...
    const shareActions = (action: ShareOptions) => {
        switch (action) {
            case 'copy':
                if (type === 'story') {
                    navigator.clipboard.writeText(`http://localhost:5173/profile/${contentId}/story`)
                } else {
    ...
    return (
        ...
        >
            ...
            >
                ...
                <ul className="flex flex-wrap gap-3 w-full overflow-y-scroll custom-scrollbar mt-2 px-4">
                    {isGettingUserFollowings
                        ? (
                            <div className="flex-center w-full h-[5rem]">
                                <Loader />
                            </div>
                        )
                        : searchResults?.map(following => (
                            <li key={following.$id} className="flex-center flex-col gap-2 cursor-pointer w-20">
                                <img
